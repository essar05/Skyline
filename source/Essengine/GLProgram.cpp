...
namespace Essengine {
  ...
  GLProgram::GLProgram(bool debugMode = false):_debugMode(debugMode) {
  ...
  void GLProgram::loadShader(ShaderType type, const std::string& shaderFilePath) {
    if (_shaderIds[type] != 0) {
      throw Essengine::ERuntimeException("Already loaded another shader of the same type.");
    }

    _shaderIds[type] = glCreateShader(type);

    if (_shaderIds[type] == 0) {
      throw Essengine::ERuntimeException("Failed to create shader id");
    }
    ...
    if (fileStream.is_open()) {
      ...
      throw Essengine::ERuntimeException("Cannot load shader file: " + shaderFilePath);
    }
  ...
  void GLProgram::compileShaders() {
    for (std::map<GLuint, GLuint>::iterator it = _shaderIds.begin(); it != _shaderIds.end(); ++it) {
      ...
      if (compileSuccess == GL_FALSE) {
        ...
        std::printf("%s\n", &errorLog[0]);
        throw Essengine::ERuntimeException("Failed to compile shader with id " + shaderId);
      }
    }
  ...
  void GLProgram::linkShaders() {
    ...
    if (isLinked == GL_FALSE) {
      ...
      std::printf("%s\n", &errorLog[0]);
      throw Essengine::ERuntimeException("Failed to link program with id " + _programId);
    }
