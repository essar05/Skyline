...
void SceneRenderer::render() {
  ...
  if(game->getGameplayScreen()->isPaused()) {
    ...
    float cameraPosX = camera->getPosition().x / camera->getZoom() - camera->getWorldScalar(game->getWidth()) / 2;
    float cameraPosY = camera->getPosition().y / camera->getZoom() - camera->getWorldScalar(game->getHeight()) / 2;
